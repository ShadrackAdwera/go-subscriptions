...
func (q *Queries) DeleteUserPackage(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, deleteUserPackage, id)
	return err
}

const getUserPackages = `-- name: GetUserPackages :many
SELECT users_packages.id, user_id, package_id, status, start_date, end_date, users.id, username, email, created_at, packages.id, name, description, price 
FROM users_packages 
